...
// ---------- AI AUDITOR (Dify workflow API or n8n webhook) ----------
document.getElementById("auditBtn").addEventListener("click", async () => {
  ...
  if (!endpoint) {
    ...
    stampWrap.innerHTML = `<div class="audit-stamp">Local Audit — no endpoint set</div>`;
    return;
  }
  ...
  try {
    ...
    if (provider === "dify") {
      const res = await fetch(endpoint, {
        ...
        body: JSON.stringify({
          // NOTE: this key must exactly match your Dify Start node's
          // input field name. Yours is named "input_text".
          inputs: { input_text: JSON.stringify(payload) },
          response_mode: "blocking",
          user: "hackathon-demo",
        }),
      });
      if (!res.ok) throw new Error("Dify returned " + res.status);
      const data = await res.json();
      // Matches the End node output variable named "summary"
      summaryText = data.data?.outputs?.summary || JSON.stringify(data.data?.outputs, null, 2);
    } else {
      // n8n webhook path
      const res = await fetch(endpoint, {
        method: "POST",
      ...
      const data = await res.json();
      summaryText = data.summary || JSON.stringify(data, null, 2);
    }

    output.textContent = summaryText;
    stampWrap.innerHTML = `<div class="audit-stamp">Audited by AI Agent</div>`;
  } catch (err) {
    output.textContent = "Could not reach the AI agent (" + err.message + "). Showing a local summary instead:\n\n"
...
// The real AI reasoning happens in the Dify/n8n workflow.
function localFallbackSummary(payload) {
  ...
  const lines = [
    ...
    "Connect the AI agent endpoint above for a real AI-generated summary.",
  ];
